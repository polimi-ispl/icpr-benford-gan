...
def main():
    ...
    parser.add_argument('--C', type=float, help='C parameter of SVM', required=True)
    parser.add_argument('--save_estimator', help='Save the estimator', action='store_true', default=False)
    ...
    print('C: {}, Total {} samples, Leave-One-Group-Out cv. Feature size: {}'.format(C, X.shape[0],
                                                                                     X.shape[1]))
    ...
    cv = cross_validate(estimator=model, X=X, y=y, groups=y_logo,
                        scoring='balanced_accuracy', cv=logo, verbose=2, return_estimator=save_estimator, n_jobs=-1)
